...
class Eventmonitor:
	...
	async def check_bot(matcher: Matcher) -> None:
		...
		try:
			(
				code,
				error,
			) = await eventmonitor.check_update(
				matcher
			)
			if error:
				logger.error(
	...
	async def check_update(matcher: Matcher):
		...
		data = (
			await update.get_latest_version_data()
		)
		if data:
	...
	async def _file_handle():
		...
		# 记录日志，表示解压文件压缩包完成
		latest_file: Path = (
			Path(update.temp_dir)
			/ os.listdir(update.temp_dir)[0]
		)
