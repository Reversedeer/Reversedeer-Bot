...
class Utils:
	def __init__(self) -> None:
		...
		self.chuo_CD_dir = {}
		self.config_path = (
			Path() / 'data/eventmonitor'
		)
		self.address = (
			self.config_path / 'config.json'
		)
		config = nonebot.get_driver().config
		self.superusers: set[int] = {
			int(uid) for uid in config.superusers
		}
		self.nickname: str = next(
			iter(config.nickname), 'Bot'
		)
		self.chuo_cd: int = getattr(
			config, 'chuo_cd', 10
		)
		self.check_bot_update: bool = getattr(
			config, 'isalive', True
		)
		self.check_txt_img: bool = getattr(
			config, 'event_img', False
		)
	...
	async def config_check(self) -> None:
		...
		# 将更新后的配置字典上传到配置文件中
		self.json_upload(
			self.address, config_dict
		)

	@staticmethod
	async def check_chuo(
		g_temp, gid: str
	) -> bool:
		"""检查戳一戳是否允许"""
		if (
			gid in g_temp
			and not g_temp[gid]['chuo']
		):
			return False
		return g_temp[gid]['chuo']

	@staticmethod
	async def check_honor(
		g_temp, gid: str
	) -> bool:
		"""检查群荣誉是否允许"""
		if (
			gid in g_temp
			and not g_temp[gid]['honor']
		):
			return False
	...
	async def check_file(
		...
		if (
			gid in g_temp
			and not g_temp[gid]['files']
		):
			return False
		return g_temp[gid]['files']

	@staticmethod
	async def check_del_user(
		g_temp, gid: str
	) -> bool:
		"""检查群成员减少是否允许"""
		if (
			gid in g_temp
			and not g_temp[gid]['del_user']
		):
			return False
	...
	async def check_admin(
		...
		if (
			gid in g_temp
			and not g_temp[gid]['admin']
		):
			return False
		return g_temp[gid]['admin']

	@staticmethod
	async def check_red_package(
		g_temp, gid: str
	) -> bool:
		"""检查运气王是否允许"""
		if (
			gid in g_temp
			and not g_temp[gid]['red_package']
		):
			return False
	...
	def write_group_data(g_temp) -> None:
		"""写入群配置"""
		with open(
			utils.address, 'w', encoding='utf-8'
		) as f:
			json.dump(
				g_temp,
				f,
