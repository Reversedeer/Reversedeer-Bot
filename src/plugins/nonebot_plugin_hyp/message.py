...
class Message:
    @staticmethod
    async def send_hyp_msg(data: dict, data_a: dict, online: str) -> str:
        """构建/hyp回复消息"""
    ...
    @staticmethod
    async def send_apikey_msg(hyp_apikey: str, ant_apikey: str) -> str:
        """构建/apikay回复消息"""
        msg: str = (
            f'当前API信息如下:\n'
    ...
    @staticmethod
    async def send_mc_msg(data: dict) -> str:
        """构建/mc回复消息"""
        error: str = data.get('e', '')
        if error:
            return error
        msg: str = f'ID: {data["name"]}\nUUID: {data["id"]}'
        return msg

    @staticmethod
    async def send_bw_msg(data: dict, data_a: dict, rank: str) -> str:
        """构建/bw回复消息"""
        error: str = data.get('e', '')
        if error:
            return error
    ...
    @staticmethod
    async def send_sw_msg(data: dict, data_a: dict, rank: str) -> str:
        """构建/sw回复消息"""
        error: str = data.get('e', '')
        if error:
            return error
