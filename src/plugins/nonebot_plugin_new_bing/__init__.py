...
bingchat = on_command("bing", aliases={"bingchat"}, priority=3, block=False)
reserve = on_command("重置会话", aliases={"重置bing"}, priority=3, block=True)
bingHelp = on_command("bing帮助", aliases={"binghelp"}, priority=3, block=True)
...
async def _(mathcer: Matcher, event: MessageEvent):
    ...
    if uid not in chat_dict:                # 如果用户id不在会话字典中, 则新建一个会话
        ...
        await bingchat.send("newbing 新会话已创建", at_sender=True)
    ...
    if len(data["item"]["messages"]) < 2:       # 如果返回的消息数量小于2, 则说明会话已经中断, 则删除当前会话
        await bingchat.send("该对话已中断, 可能是被bing掐了, 请重置会话", at_sender=True)
        _ = await new_chat_(event=event, matcher=mathcer, user_id=uid)
        return
    ...
    try:                                # 尝试发送回复
        if maxConversation > currentConversation:   # 如果当前对话数小于最大对话数, 则正常发送
            ...
            await bingchat.send(repMessage + f"\n\n当前{currentConversation} 共 {maxConversation}", at_sender=True)
            await bingchat.send("达到对话上限, 正帮你重置会话\n使用重置会话+balanced/creative/precise 来切换会话模式", at_sender=True)
...
async def _(matcher: Matcher, event: MessageEvent, msg: Message = CommandArg()):
    ...
    if msg not in ["balanced", "creative", "precise"]:
        ...
        # 返回提示
        await reserve.finish("缺少参数或参数错误, 请使用 balanced 或者 creative 或 precise, 这里只能默认给你重置成balanced")
    else:
        # 重置会话并返回提示
